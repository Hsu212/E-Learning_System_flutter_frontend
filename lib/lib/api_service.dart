 ...
 // --- AUTHENTICATION ---
  ...
  Future<app_models.User?> signIn(String userId, String password) async {
    ...
    // [NEW] Default Student Login
      ...
      email = 'student@tdtu.edu.vn';     // Must match what you added in Supabase
    ...
    try {
      ...
      if (response.user != null) {
        ...
        if (user == null) {
          ...
          // If they logged in as 'student', role is 'student'
    ...
    try {
      ...
      for (var c in data) {
        ...
        courses.add(app_models.Course(
          ...
          instructorId: c['instructor_id'] ?? '', // Handle potential nulls
  ...
  Future<app_models.Course?> createCourse(app_models.Course course) async {
    try {
      // IMPORTANT: We use the current AUTHENTICATED user ID
      ...
      final response = await _supabase.from('courses').insert({
        ...
        'instructor_id': realUserId, // This links it to YOU
...
Future<List<app_models.User>> getGroupMembers(String groupId) async {
    try {
      // Join 'group_members' with 'users' to get actual names/emails
      final data = await _supabase
          .from('group_members')
          .select('users(*)') // (*) fetches the full user object linked
          .eq('group_id', groupId);

      List<app_models.User> members = [];
      for (var row in data) {
        final u = row['users']; // This is the linked user data
        if (u != null) {
...
Future<List<app_models.Classwork>> getClassworks(String courseId, String userId, String role, String? groupId) async {
    try {
      // 1. Fetch Classworks with nested questions
      // We join 'quiz_questions' and then 'question_bank' to get the actual text
      var query = _supabase
          .from('classworks')
      ...
      for (var e in (data as List)) {
        // [Fix 2.6] Scoping Enforcement
        // If student, check if this classwork is scoped to a group they are NOT in
        bool isScoped = (e['classwork_scopes'] as List).isNotEmpty;
        bool userAllowed = true;
        
        if (role == 'student' && isScoped) {
          userAllowed = false;
          // Check if my group is in the allowed list
          for (var scope in e['classwork_scopes']) {
            if (scope['group_id'] == groupId) userAllowed = true;
          }
        }

        if (!userAllowed) continue; // Skip this assignment
        ...
        results.add(app_models.Classwork(
          ...
          score: e['max_points'],
          questions: parsedQuestions, // Populated!
        ));
      }
      return results;
    } catch (e) {
  ...
  Future<List<List<dynamic>>> getCourseGradesCSV(String courseId) async {
    ...
    rows.add(["Student ID", "Name", "Email", "Total Score"]); // Header
  ...
  Future<app_models.Classwork?> createClasswork(
    ...
    try {
      ...
      // 2. Insert Scoping (Which groups can see this?)
      ...
      // 3. If Quiz, Generate Questions from Bank (Random Selection)
      if (w.type == app_models.ClassworkType.quiz) {
        await _generateQuizQuestions(newId, w.courseId, easyCount, mediumCount, hardCount);
      }

      // 4. Send Notifications to Students in those groups (Requirement No. 4)
      _notifyStudents(w.courseId, groupIds, "New ${w.type.name}: ${w.title}");

      return w; // In a real app, return the fully re-fetched object
    } catch (e) {
  ...
  // --- NEW: Quiz Generation Logic (Requirement No. 3) ---
  Future<void> _generateQuizQuestions(String quizId, String courseId, int e, int m, int h) async {
    // Note: In a real app, you filter by course_code, here we use courseId for simplicity
  ...
  // --- NEW: Private Messaging (Requirement No. 4) ---
  ...
  // --- NEW: Tracking Views (Requirement No. 3) ---
  Future<void> logView(String userId, String contentId, String type) async {
    await _supabase.from('content_views').insert({
      'user_id': userId,
      'content_id': contentId,
      'action_type': type // 'view' or 'download'
    });
  }

  // --- NEW: Notifications Helper ---
  Future<void> _notifyStudents(String courseId, List<String> groupIds, String msg) async {
    // 1. Get Students in these groups
    // This is complex in SQL, strictly simplified here:
    // Insert notification for every student found in group_members for these groups
...
Future<app_models.User?> fetchUserByStudentId(String studentId) async {
  ...
  // Note: We use the Node URL getter logic you already have
...
Future<void> createSemester(String name) async {
  try {
    await _supabase.from('semesters').insert({
      'name': name,
      // Ensure your DB has a 'created_at' or similar if needed
    });
  } catch (e) {
...
// --- GROUP MANAGEMENT ---
  ...
  // 1. Create a Group (e.g., "Group 1" for Course X)
  ...
  // 2. Fetch Groups for a specific Course
  ...
  Future<List<int>> getWeeklyEngagement(String instructorId) async {
    try {
      ...
      // FIXED: Use proper list syntax for 'in' filter to avoid syntax errors
      final data = await _supabase
          .from('submissions') 
          .select('submitted_at')
          .filter('course_id', 'in', courseIds) // Pass the List directly, not a String
  ...
  // 3. Assign Student to a Group
  // Note: PDF says a student can only be in ONE group per course.
  // You should check if they are already in a group for this course before adding.
